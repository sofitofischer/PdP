...
hierro :: Int -> Palo
hierro n habilidad = UnTiro (fuerzaJugador habilidad * n) (div (precisionJugador habilidad) n) (n - 3)

type Palo = Habilidad -> Tiro
palos :: [Palo] 
palos = [putter, madera] ++ map hierro [1..10]

{- ej 2) Definir la función golpe que dados una persona y un palo, obtiene el tiro resultante de usar ese palo con las habilidades de la persona.
Por ejemplo si Bart usa un putter, se genera un tiro de velocidad = 10, precisión = 120 y altura = 0. -}

golpe :: Jugador -> Palo -> Tiro
golpe jugador palo = palo (habilidad jugador)

{- ej 3) Lo que nos interesa de los distintos obstáculos es si un tiro puede superarlo, y en el caso de poder superarlo, cómo se ve afectado dicho tiro por el obstáculo. En principio necesitamos representar los siguientes obstáculos:
...
-- Tiros de ejemplo
tiro1, tiro2 :: Tiro
tiro1 = UnTiro 100 100 0
tiro2 = UnTiro 10 95 0

{- ej 4) 
A) Definir palosUtiles que dada una persona y un obstáculo, permite determinar qué palos le sirven para superarlo. 
...
cantidadDeObstaculosConsecutivos :: [Obstaculo] -> Tiro -> Int 
cantidadDeObstaculosConsecutivos obstaculos tiro = length (takeWhile (`puedeSuperar` tiro) obstaculos)

paloMasUtil :: Jugador -> [Obstaculo] -> Palo
paloMasUtil jugador obstaculos = maximoSegun (cantidadDeObstaculosConsecutivos obstaculos . golpe jugador) palos 

{- ej 5) Dada una lista de tipo [(Jugador, Puntos)] que tiene la información de cuántos puntos ganó cada niño al finalizar el torneo, se pide retornar la lista de padres que pierden la apuesta por ser el “padre del niño que no ganó”. Se dice que un niño ganó el torneo si tiene más puntos que los otros niños. -}
...
gano puntos (jugador, puntosJugador) = all ((< puntosJugador) . snd) (filter (/= (jugador, puntosJugador)) puntos)